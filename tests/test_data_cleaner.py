...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        # Original DataFrame no fue modificado
        self.assertEqual(df["name"].iloc[0], " Alice ")
        self.assertEqual(df["name"].iloc[3], " Carol  ")

        # Resultado tiene los strings sin espacios
        ...
        # Columna "city" no fue modificada
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        """
        df = make_sample_df()
        df = pd.concat([df, pd.DataFrame({"name": ["X", "Y"], "age": [27, 33], "city": ["SCL", "LPZ"]})], ignore_index=True)
        cleaner = DataCleaner()
